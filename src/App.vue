		...
		<v-container>
			<v-container>
				...
				<v-row>
					<v-container>
				<v-img
              :src="require('../src/assets/pokedexx.png')"
              class="my-3"
              contain
              height="250"
            />
				<h1 class="text-center white--text mb-b style=font-size: 3rem"> Pokedéx Feita por João Victor </h1>
					</v-container>
				</v-row>
				<v-text-field
					v-model="search"
				...
				<v-container>
					<v-row class="d-flex align-center">
						...
						<v-col cols="8">
							...
							<v-chip label v-for="type in selected_pokemon.types" :key="type.slot" class="mr2">{{ type.type.name }}
							</v-chip>
							...
							<v-chip label classe="ml-3"> Peso {{ (selected_pokemon.weight * 0.45359237).toFixed(0) }} kgs </v-chip>
